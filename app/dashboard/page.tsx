...
import { debugPermissions } from "@/lib/utils/debug-permissions";
...
export default async function DashboardPage() {
  ...
  // Debug permissions
  const debugInfo = await debugPermissions();
  ...
  console.log("Email role:", emailRole);
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Your account is ready, but needs role assignment
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Your account needs to be assigned a role to access dashboard
            features
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
            ...
            <ul className="text-sm text-yellow-700 dark:text-yellow-300 space-y-1">
              <li>
                • Super Admin and Admin roles are assigned based on email
                configuration
              </li>
